...
本リポジトリは **MIT License** のもとで公開されています。商用・非商用問わず誰でも自由に利用・改変・フォーク・配布が可能です。
...
`sample_qr.wasm` と `sample_wrapper.js` を使用し、2D マトリクス生成を 200 回連続実行して動作精度と速度を比較検証した実測値です：

| 検証項目 | オリジナル（未難読化） | 難読化後 (`--export-symbols` 保護) | 判定・差分 |
| :--- | :--- | :--- | :--- |
